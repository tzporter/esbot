...
class ChatService:
    ...
    def generate_quiz(self, session_id: int, topic: str):
        ...
        try:
            ...
            for item in quiz_data.get("quiz", {}).get("questions", []):
                qr.add_item(
                    question_text=item["question"], correct_answer=item["answer"]
                )
    ...
    def evaluate_answer(
        self, question_text: str, correct_answer: str, user_answer: str
    ) -> dict:
        try:
            ...
            return {
                "is_correct": False,
                "feedback": "Feedback currently unavailable due to AI service error.",
            }
