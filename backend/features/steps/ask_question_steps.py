...
def step_impl(context, explanation):
    context.mock_ai = patch.object(
        ai_provider, "get_explanation", return_value=explanation
    )
...
def step_impl(context):
    context.mock_ai = patch.object(
        ai_provider, "get_explanation", side_effect=ConnectionError()
    )
    context.mock_ai.start()
    ...
    with Session(engine) as db_session:
        msg = db_session.exec(
            select(ChatMessage).where(
                ChatMessage.role == role, ChatMessage.content == content
            )
        ).first()
...
@then("no ChatMessage should be created in the database")
def step_impl(context):
    with Session(engine) as db_session:
        messages = db_session.exec(
            select(ChatMessage).where(ChatMessage.content == "")
        ).all()
        assert len(messages) == 0


@then(
    'a ChatMessage should be created with role "assistant" and content containing "{keyword}"'
)
